...
+
有關 `storage aggregate relocation start`、 `override-vetoes` 和 `override-destination-checks` 選項的詳細資訊、請參閱link:other_references.html["參考資料"]以連結至 _ONTAP 9 Command reference_。
...
* 在第 3 階段期間、會將 Aggregate 從 node2 重新配置到 node3。部分重新配置的 Aggregate 以 node1 為主節點。例如、這類 Aggregate 可稱為 aggr_node_1。如果在第 3 階段期間重新配置 aggr_node_1 失敗、且無法強制重新配置、則 Aggregate 會留在 node2 上。
* 在第 4 階段之後，當 node2 被 node4 取代時。當 node2 被取代時，aggr_node_1 將以 node4 作為其主節點而不是 node3 上線。
