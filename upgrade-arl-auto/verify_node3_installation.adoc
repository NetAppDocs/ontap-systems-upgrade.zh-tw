...
如果您在升級的系統上使用 NetApp Aggregate Encryption（NAE）或 NetApp Volume Encryption（NVE）來加密磁碟區，則必須將加密組態同步到新節點。如果您未還原 key-manager，則在使用 ARL 將 node1 彙總從 node2 重新配置到 node3 時，加密磁碟區將會離線。
