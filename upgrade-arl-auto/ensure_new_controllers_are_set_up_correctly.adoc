...
summary: 啟用HA配對、驗證node3和node4儲存設備存取、並ONTAP 使用「系統控制器更換」命令、將執行版本號從9.5升級至9.7的控制器後、確認Aggregate擁有權。 
---
= 確認新的控制器已正確設定
:hardbreaks:
:allow-uri-read: 
...
. [[unset_mastwait_system_commands]]如果在組態中使用自我加密磁碟機、且您已設定 `kmip.init.maxwait` 變數至 `off` （例如、in link:install_boot_node4.html#step27["安裝並開機節點4、步驟27"]）、您必須取消設定變數：
+
