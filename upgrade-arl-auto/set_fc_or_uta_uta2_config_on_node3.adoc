...
. 執行下列其中一項動作：
+
** 如果 node3 有 UTA/UTA2 卡或 UTA/UTA2 板載連接埠，請前往 <<check-config-uta-uta2-node3,檢查並設定節點3上的UTA/UTA2連接埠>>。
** 如果 node3 沒有 UTA/UTA2 卡或 UTA/UTA2 內建連接埠，請跳過 <<check-config-uta-uta2-node3,檢查並設定節點3上的UTA/UTA2連接埠>> 並前往 link:reassign-node1-disks-to-node3.html["將 node1 磁碟重新指派給 node3"]。
