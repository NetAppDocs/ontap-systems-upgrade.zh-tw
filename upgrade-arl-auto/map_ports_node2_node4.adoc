...
[role="lead"]
您必須驗證 node2 上的實體連接埠是否與 node4 上的實體連接埠正確對應。這樣才能確保升級後 node4 能夠與叢集中的其他節點以及網路進行通訊。
...
因此，如果節點 2 上的實體連接埠與節點 4 上的實體連接埠不直接對應，則需要進行軟體組態變更才能在開機後還原叢集、管理和網路連線。此外，如果節點 2 上的叢集連接埠與節點 4 上的叢集連接埠不直接對應，則節點 4 在重新開機後可能不會自動重新加入仲裁，直到進行軟體組態變更，將叢集 LIF 託管在正確的實體連接埠上。
...
+
請參閱 link:other_references.html["參考資料"] 以連結至_網路管理_或 _ONTAP 9 Command reference_，並參閱_在 LIF 上配置容錯移轉設定_以取得更多資訊。
