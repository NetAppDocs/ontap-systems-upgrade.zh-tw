...
summary: 使用ONTAP 「系統控制器更換」命令、參考支援執行《支援》從9.5版升級至9.7版的控制器所需的內容和網站。 
---
= 參考資料
:hardbreaks:
:allow-uri-read: 
...
| link:https://docs.netapp.com/us-en/ontap-cli/index.html["ONTAP 9 指令參考"^] | 說明支援的 ONTAP 命令的語法和使用方式。 
...
| link:https://docs.netapp.com/us-en/ontap-systems-upgrade/upgrade-arl-auto-app-9151/index.html["使用「system controller replace」指令升級至 ONTAP 9.15.1 及更新版本中引入的控制器硬體"^] | 說明使用「system controller replace」命令，以不中斷營運的方式升級至 ONTAP 9.15.1 及更新版本所引入之控制器所需的集合體重新定位程序。 
