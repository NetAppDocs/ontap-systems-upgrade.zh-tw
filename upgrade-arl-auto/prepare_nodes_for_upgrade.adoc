...
+
** 「Vserver服務名稱服務DNS節目」
** 「網路介面show -curr-node-nodename_-role cluster、intercluster、node-mgmt、clustermgmt、data」
** 「網路連接埠show -nodelame_-type Physical」
** 「ervice處理器show -Node *-instance」
** 「網路FCP介面卡顯示-node_node_name_」
** 「網路連接埠ifgrp show」
** 「網路連接埠VLAN show」
** 「ystem nodem show -instance -nodename_」（系統節點顯示-instance節點_nodename_）
** "run -node_node_name_ sysconfig"
** 「torage Aggregate show -nodename_」
** 「Volume show -node_node_name_」
** 「網路介面容錯移轉群組顯示」
** 「torage Array config show -switch_switch_name_」
** 「System license show -Oner_node_name_」（系統授權顯示擁有者節點名稱）
** 「torage加密磁碟顯示」
