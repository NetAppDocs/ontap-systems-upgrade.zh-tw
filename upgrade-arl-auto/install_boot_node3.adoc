...
+
如果您沒有 FC 或 UTA/UTA2 組態，請執行，link:reassign-node1-disks-to-node3.html#reassign-node1-node3-step1["將 node1 磁碟重新指派給 node3 ，步驟 1"]以便 node3 能夠辨識 node1 的磁碟。

. [[auto9597_istall3_step29]] 對於 MetroCluster 組態， V 系列系統和系統，以及連接至儲存陣列的 FlexArray 虛擬化軟體，請前往link:reassign-node1-disks-to-node3.html#reassign-node1-node3-step1["將 node1 磁碟重新指派給 node3 ，步驟 1"]。
