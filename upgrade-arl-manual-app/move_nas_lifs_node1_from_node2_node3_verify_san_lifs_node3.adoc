...
.關於這項工作
遠端 LIF 在升級過程中處理 SAN LUN 的流量。升級期間，無需移動 SAN LIF 以確保叢集或服務的正常運作。除非需要將 SAN LIF 對應到新連接埠，否則不會移動它們。節點 3 上線後，您需要驗證 LIF 是否運作正常並位於正確的連接埠上。
...
.接下來呢？
將節點 1 擁有的 NAS 資料 LIF 從節點 2 移動到節點 3，並在節點 3 上驗證 SAN LIF 後，
