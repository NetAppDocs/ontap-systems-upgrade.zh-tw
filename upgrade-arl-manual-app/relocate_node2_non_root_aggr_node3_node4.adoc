...
如需儲存 Aggregate 重新配置命令的詳細資訊、請參閱link:other_references.html["參考資料"]以連結至_使用 CLI 進行磁碟和 Aggregate 管理_和_ ONTAP 9 命令參考_。
...
.接下來呢？
將節點 2 的非根 Aggregate 從節點 3 重新配置到節點 4 後，link:manage_authentication_kmip.html["使用 KMIP 伺服器管理驗證"]。
