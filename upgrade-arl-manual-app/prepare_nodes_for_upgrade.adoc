...
+
如果 node1 和 node2 都不擁有自己是目前擁有者（但不是主擁有者）的 Aggregate、則系統會傳回類似下列範例的訊息：
...
+
有關  `network interface revert` 命令的詳細資訊，請參閱 link:other_references.html["參考資料"] 以連結至 _ONTAP 9 Command reference_。
...
NOTE: 您可以使用這些資訊來識別您可能想要傳輸到節點 3 或節點 4 的零件或配件。
...
+
VLAN 是透過實體連接埠配置的。如果實體連接埠發生變化，則需要在後續流程中重新建立 VLAN。
...
+
如果 SVM 既未設定 iSCSI 也未設定 FC，則該命令會顯示類似下列範例的訊息：
...
+
如果為 SVM 設定了 iSCSI 或 FC，則該命令會顯示類似於下列範例的訊息：
...
. [[man_prepare_nodes_step28]]完成下列子步驟：
+
...
.. 記錄有關SP的IP位址和其他資訊。
+
您可能需要將原始系統中遠端管理設備（在本例中為 SP）的網路參數重用於新節點上的 SP。有關 SP 的詳細資訊，請參閱link:other_references.html["參考資料"]以連結至《系統管理參考》和《ONTAP 9 命令參考》。



. [[man_prepare_nodes_step29]]如果您希望新節點具有與原始節點相同的許可功能，請輸入以下命令以查看原始系統上的叢集許可證：
+
「系統授權顯示擁有者*」
...
+
有關 AutoSupport 的詳細資訊，請參閱 link:other_references.html["參考資料"] 以連結至《系統管理參考》和《ONTAP 9 命令參考》。
...
.接下來呢？
完成節點升級準備工作後，link:manage_authentication_okm.html["使用 Onboard Key Manager 管理驗證金鑰"]。
