...
.關於這項工作
如果節點 4 上的ONTAP版本與節點 2 上的ONTAP版本不同，則必須對節點 4 進行網路啟動。安裝 node4 後，從儲存在 Web 伺服器上的ONTAP 9 映像啟動它。然後，您可以按照說明將正確的檔案下載到啟動媒體設備，以便後續系統啟動。 link:prepare_for_netboot.html["準備netboot"]

但是，如果 node4 上的ONTAP版本與 node2 上的ONTAP版本相同或更高，則無需對 node4 進行網路啟動。
...
+
輸入您先前在程序中記錄的通關密語和備份資訊。請參閱link:manage_authentication_okm.html["使用 Onboard Key Manager 管理驗證金鑰"]。
...
+
此 `disk reassign`命令僅重新指派 `_node2_sysid_`為目前擁有者的磁碟。
...
.接下來呢？
安裝並啟動 node4 後，link:set_fc_uta_uta2_config_node4.html["在節點 4 上設定 FC 或 UTA/UTA2 配置"]。
