...
NOTE: 您的系統可能沒有叢集間 LIF。您在節點配對的其中一個節點上只有一個叢集管理 LIF。叢集管理 LIF 會顯示在 _Record node1 port information_ 的 link:record_node1_information.html#step1["步驟1."] 範例輸出中。
...
.接下來呢？
記錄節點 2 資訊後，link:retire_node2.html["退役 node2"]。
