...
.步驟
. [[relocate-step1]]透過完成以下子步驟來重新定位非根聚合：
+
.. 將權限層級設為進階：
+
"進階權限"
...
請參閱 link:other_references.html["參考資料"] 以連結至 _Disk and aggregate management with the CLI_ 內容和 _ONTAP 9 Command reference_，以取得有關儲存 Aggregate 重新配置命令的詳細資訊。
...
.接下來呢？
將非根 Aggregate 從 node1 重新配置到 node2 後link:move_nas_lifs_node1_node2.html["將節點 1 擁有的 NAS 資料 LIF 移至節點 2"]。
