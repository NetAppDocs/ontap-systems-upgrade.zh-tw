...
[role="lead"]
您必須確保節點 2 上的實體連接埠與節點 4 上的實體連接埠正確對應。這樣，升級後節點 4 才能與叢集中的其他節點以及網路進行通訊。
...
| 9.12.1 或更新版本 | 跳過此步驟、前往 <<man_map_2_Step3,步驟3.>>。 
|===
+
...
.. 在執行可達性修復時，ONTAP 會嘗試將連接埠放置在正確的廣播網域中。但是，如果 ONTAP 無法確定連接埠的可達性，且該連接埠不屬於任何現有廣播網域，ONTAP 會嘗試為這些連接埠建立新的廣播網域。如有需要，您可以刪除新建立的廣播網域，前提是其所有成員連接埠都將成為介面群組的成員連接埠。刪除廣播網域：
+
...
. [[man_map_2_Step5]]從 ONTAP 9.8 開始，如果連接埠在網路連接埠可連線性修復程序期間於廣播網域之間移動，ONTAP 會自動修改 LIF 的主連接埠。如果 LIF 的主連接埠被移至另一個節點或未指派，則該 LIF 會顯示為已移位的 LIF。還原主連接埠已不存在或已重新配置至另一個節點之已移位 LIF 的主連接埠。
+
...
.接下來呢？
將連接埠從節點 2 對應到節點 4 之後，link:move_nas_lifs_node2_from_node3_node4_verify_san_lifs_node4.html["將節點 2 擁有的 NAS 資料 LIF 從節點 3 遷移到節點 4，並驗證節點 4 上的 SAN LIF"]
