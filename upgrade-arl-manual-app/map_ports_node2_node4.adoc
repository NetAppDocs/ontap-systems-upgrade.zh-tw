...
+
「網路介面移轉-vserver <vserver_name>-lif<lif_name> source-node2 -dite-node2 -dite-port <port_name>」
...
+
「網路連接埠廣播網域移除連接埠-IPSpace叢集-broadcast網域叢集-ports <node2：port>」
...
+
"ifgrp add-port -node_name -ifgrp <ifgrp_port>-port <port_name>

+
從介面群組連接埠移除成員連接埠：

+
"ifgrp remove-port -node_name -ifgrp <ifgrp_port>-port <port_name>
...
.. 檢查是否有任何運作中斷的生命：
+
「網路介面show -vserver <vserver_name>-STATUS-oper down」

.. 修改任何需要修改的生命期、使其具有不同的主連接埠：
+
「網路介面修改-vserver <vserver_name>-lif<lif_name>-home-port <home_port>`
