...
+
「網路介面移轉-vserver <vserver_name>-lif<lif_name>-source-Node <node1>-dite-node<node1>-destite-port <port_name>」
...
+
「ifgrp disfile-port -node_<node_name>_-ifgrp <ifgrp_port>-port <port_name>」

.. 視需要刪除並重新建立VLAN連接埠。刪除VLAN連接埠：
+
「VLAN刪除節點<node_name>-vlan-name <vla_port>_」
...
.. 還原每個LIF的主連接埠：
+
「顯示介面還原-vserver <vserver_name>-lif-name <LIF_name>」
