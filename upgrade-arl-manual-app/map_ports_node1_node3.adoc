...
[role="lead"]
您必須確保 node1 上的實體連接埠與 node3 上的實體連接埠正確對應。這樣，升級後 node3 才能與叢集中的其他節點以及網路進行通訊。
...
.. 在執行可達性修復時，ONTAP 會嘗試將連接埠放置在正確的廣播網域中。但是，如果 ONTAP 無法確定連接埠的可達性，且該連接埠不屬於任何現有廣播網域，ONTAP 會嘗試為這些連接埠建立新的廣播網域。如有需要，如果所有新建立的廣播網域的成員連接埠都已成為介面群組的成員連接埠，則可以刪除這些廣播網域。刪除廣播網域：
+
...
. [[man_map_1_step5]]從 ONTAP 9.8 開始，如果連接埠在網路連接埠可連線性修復程序期間於廣播網域之間移動，ONTAP 會自動修改 LIF 的主連接埠。如果 LIF 的主連接埠被移至另一個節點或未指派，則該 LIF 會顯示為已移位的 LIF。還原主連接埠已不存在或已重新配置至另一個節點之已移位 LIF 的主連接埠。
+
...
.接下來呢？
將連接埠從 node1 對應到 node3 之後，link:move_nas_lifs_node1_from_node2_node3_verify_san_lifs_node3.html["將節點 1 擁有的 NAS 資料 LIF 從節點 2 遷移到節點 3，並驗證節點 3 上的 SAN LIF"]。
