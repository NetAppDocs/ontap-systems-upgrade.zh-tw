...
+
您可以將輸出與您捕獲的輸出進行比較。link:prepare_nodes_for_upgrade.html#man_prepare_nodes_step29["步驟29"]準備節點進行升級。
...
+
前往link:other_references.html["參考資料"]連結至 _System Administration Reference_ 以取得有關 SP 的資訊，以及 _ONTAP 9 Command reference_ 以取得有關 `system service- processor network modify`命令的詳細資訊。
...
.接下來呢？
確認新控制器設定正確後，link:set_up_storage_encryption_new_controller.html["在新的控制器模組上設定 Storage Encryption"]。
