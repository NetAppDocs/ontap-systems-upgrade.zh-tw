...
如需儲存 Aggregate 重新配置命令的詳細資訊、請前往link:other_references.html["參考資料"]連結至 _Disk and aggregate management with the CLI_ 和 _ONTAP 9 Command reference_ 。
...
.接下來呢？
將非根 Aggregate 從節點 2 重新配置到節點 3 後，link:move_nas_lifs_node2_node3.html["將節點 2 擁有的 NAS 資料 LIF 移至節點 3"]。
