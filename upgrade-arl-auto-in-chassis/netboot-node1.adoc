...
除非將替換控制器模組安裝到機箱中並通電，否則無法檢查安裝在替換控制器模組啟動媒體上的ONTAP版本。替換系統啟動媒體上的ONTAP版本必須與要升級的舊系統上執行的ONTAP版本相同，且啟動媒體上的主啟動映像和備份啟動映像必須相符。若要確認升級所需的最低ONTAP版本，請參閱link:decide_to_use_the_aggregate_relocation_guide.html#supported-systems-in-chassis["支援的系統對照表"]。
