...
除非將替換控制器模組安裝到機箱中並通電，否則無法檢查其啟動媒體上安裝的 ONTAP 版本。替換系統啟動媒體上的 ONTAP 版本必須與要升級的舊系統上執行的 ONTAP 版本相同，啟動媒體上的主啟動映像和備份啟動映像也必須相符。若要驗證升級所需的最低支援 ONTAP 版本，請參閱link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["支援的系統對照表"]。
...
.開始之前
* 確認您可以使用系統存取HTTP伺服器。
* 請從 _NetApp Support Site_ 下載適用於您系統的必要系統檔案和正確版本的 ONTAP。請參閱link:other-references.html["參考資料"]以連結至 _NetApp Support Site_。
...
.步驟
. 請參閱 link:other-references.html["參考資料"] 以連結至 _NetApp Support Site_ 下載用於執行系統 netboot 的檔案。
...
. 清除啟動媒體上的所有先前配置。
+
.. 在出現以下提示時，執行  `wipeconfig` 命令。然後按 Enter 鍵：
+
[listing]
----
Please choose one of the following:
...
.接下來呢？
在對 node1 進行網路開機後，link:cable-node1-for-shared-cluster-HA-storage.html["用於共享叢集 HA 和儲存設備的纜線 node1"]。
