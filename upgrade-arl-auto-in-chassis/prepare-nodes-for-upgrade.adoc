...
啟動控制器更換程序並執行預先檢查。如有必要，請判斷使用中的自我加密磁碟機類型。
...
| 映像版本檢查 | 檢查被替換的節點是否運行相同版本的 ONTAP。如果 ONTAP 映像版本不同，則任務失敗。新節點必須安裝與原始節點相同版本的 ONTAP 9.x。如果新節點安裝了不同版本的 ONTAP，則需要在安裝後對新控制器進行 netboot。有關如何升級 ONTAP 的說明，請參閱 link:other-references.html["參考資料"] 以連結至 _Upgrade ONTAP_。 
...
| 磁碟狀態檢查 | 如果任何被替換的節點缺少磁碟或磁碟發生故障，則任務失敗。如果缺少任何磁碟，請參閱link:other-references.html["參考資料"]連結至「使用 CLI 進行磁碟和 Aggregate 管理」、「使用 CLI 進行邏輯儲存管理」和「HA 配對管理」，以設定 HA 配對的儲存。 
