...
[[step-1-verify-that-node2-is-in-quorum]]
== 步驟 1：驗證 node2 是否在仲裁中

確認 node2 處於仲裁狀態。然後，繼續執行控制器更換作業。
...
. 控制器更換處於暫停狀態後，繼續執行 <<restore_network_configuration_on_node2,還原節點2上的網路組態>>。




[[step-2-restore-network-configuration-on-node2]]
[[restore_network_configuration_on_node2]]
== 步驟 2：還原 node2 上的網路組態
...
.接下來呢？
驗證 node2 安裝後，link:restore-key-manager-config-node2.html["在節點 2 上還原金鑰管理程式組態"]。
