...
summary: 使用 ARL 升級同一機箱中的控制器型號時，更換對應的替換系統模組後，必須對 node2 進行網路開機。 
---
= netboot node2
:hardbreaks:
:allow-uri-read: 
...
.接下來呢？
在對節點 2 進行網路啟動後，link:cable-node2-for-shared-cluster-HA-storage.html["為共享叢集 HA 和儲存設備連接 node2 纜線"]。
