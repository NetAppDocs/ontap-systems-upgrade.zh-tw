...
[role="lead"]
安裝了替換模組的 Node1 現在可以啟動了。支援的替換模組列在 link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["支援的系統對照表"] 中。
...
. 當節點在啟動選單處停止時，透過在 node1 上執行以下命令將舊的 node1 磁碟重新指派給替換 node1：
+
...
.接下來呢？
使用替換系統模組啟動 node1 後，link:verify-node1-installation.html["驗證 node1 安裝"]。
