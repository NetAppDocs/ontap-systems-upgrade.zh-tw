...
如果您要升級到雙節點無交換器叢集組態，則可以跳過本節並前往 link:move-non-root-aggr-and-nas-data-lifs-back-to-node2.html["將非根Aggregate和NAS資料lifs移回節點2"]。
...
| `Up` | 前往 link:move-non-root-aggr-and-nas-data-lifs-back-to-node2.html["將非根Aggregate和NAS資料lifs移回節點2"]。 
...
.接下來呢？
驗證 RCF 組態後link:move-non-root-aggr-and-nas-data-lifs-back-to-node2.html["將非根 Aggregate 和 NAS 資料 LIF 移回節點 2"]。
