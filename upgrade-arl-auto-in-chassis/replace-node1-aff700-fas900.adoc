...
---
= 更換節點 1 的 AFF A700 或 FAS9000 控制器和 NVRAM 模組
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
將節點 1 的 AFF A700 或 FAS9000 控制器和 NVRAM 模組更換為 ASA A900 、 AFF A900 或 FAS9500 控制器和 NVRAM 模組。
...
拆下並移除節點 1 AFF A700 或 FAS9000 控制器模組。
...
解鎖並移除節點 1 AFF A700 或 FAS9000 NVRAM 模組。
...
在節點 1 中安裝、連接 ASA A900、AFF A900 或 FAS9500 NVRAM 模組。
...
[[step-4-install-the-asa-a900-aff-a900-or-fas9500-controller-module-in-node1]]
== 步驟 4：在節點 1 中安裝 ASA A900、AFF A900 或 FAS9500 控制器模組

在節點 1 中安裝、連接纜線並連接 ASA A900、AFF A900 或 FAS9500 控制器模組。
...
. 中斷 AUTOBOOT 後、node1 會停留在 LOADER 提示字元。如果您未能及時中斷 AUTOBOOT 且 node1 開始開機、請等待提示字元並輸入 Ctrl-C 以進入開機功能表。節點停留在開機功能表後、請使用選項  `8` 重新啟動節點、並在重新啟動期間中斷 AUTOBOOT 。
...
.接下來呢？
安裝 ASA A900、AFF A900 或 FAS9500 控制器和 NVRAM 模組後，link:netboot-node1.html["netboot node1"]。
