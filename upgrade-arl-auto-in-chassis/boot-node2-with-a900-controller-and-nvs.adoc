...
[role="lead"]
安裝了替換模組的 Node2 現在可以啟動了。支援的替換模組列在 link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["支援的系統對照表"] 中。


NOTE: 只有在您透過交換系統模組進行升級時，才能移動主控台和管理連線。

.步驟
. [[boot_node2_step1]]如果您安裝了 NetApp 儲存加密 (NSE) 驅動器，請執行下列步驟：
+
...
+
輸入您先前在程序中記錄的通關密語和備份資訊。請參閱 link:complete-preparation-for-upgrade.html#manage-storage-encryption-using-okm["使用 Onboard Key Manager 管理儲存加密"]。
...
. 當節點在啟動選單處停止時，透過在 node2 上執行以下命令將舊的 node2 磁碟重新指派給替換 node2：
+
...
.. 在載入程式提示下停止節點。
.. 檢查並重設中所述的儲存加密bootargs <<boot_node2_step1,步驟1.>>。
.. 在載入程式提示下、開機：
+
Boot_ONTAP
...
.接下來呢？
使用替換系統模組啟動 node2 後，link:verify-node2-installation.html["驗證 node2 安裝"]
