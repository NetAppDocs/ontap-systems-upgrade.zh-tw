...
* 如果您偏好使用不同的方法來升級控制器硬體，並且願意進行磁碟區移動，請參閱link:other-references.html["參考資料"]以連結至_透過移動磁碟區或儲存設備進行升級_。


.相關資訊
請參閱link:other-references.html["參考資料"]以連結至 _ONTAP 9 文件_。

.接下來呢？
在決定使用此 ARL 程序之後，link:required-tools-and-documentation.html["瞭解所需的工具和文件"]。
