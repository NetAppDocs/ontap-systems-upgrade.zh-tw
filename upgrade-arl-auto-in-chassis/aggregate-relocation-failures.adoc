...
+
有關  `storage aggregate relocation start`、 `override-vetoes` 和  `override-destination-checks` 選項的詳細資訊、請參閱 link:other-references.html["參考資料"] 以連結至 _ONTAP 9 命令參考_。
...
+
當 node2 被替換時、aggr_node_1 會以 node1 作為其主節點而非 node2 上線。
...
+
若要識別未正確重新定位的 Aggregate，請參考您在 link:prepare-nodes-for-upgrade.html["準備節點以進行升級"] 區段中取得的 node1 主擁有者 Aggregate 清單，並將其與上述命令的輸出進行比較。

. 將步驟 1 的輸出與您在 link:prepare-nodes-for-upgrade.html["準備節點以進行升級"] 區段中為 node1 擷取的輸出進行比較，並記下任何未正確重新配置的集合體。
. 重新定位節點2上留下的集合體：
+
