...
* 如果您是從現有系統升級，並且停機時間包含在內，link:decide_to_use_the_aggregate_relocation_guide.html#supported-systems-in-chassis["支援的系統對照表"]您可以透過行動儲存裝置或聯絡技術支援來升級控制器硬體。請參閱link:other_references.html["參考資料"]連結到 _透過移動卷或儲存進行升級_。
