...
* 此程序較為複雜，並假設您具備進階 ONTAP 管理技能。在開始升級之前，您應該閱讀並理解 link:guidelines_for_upgrading_controllers_with_arl.html["控制器升級指南"] 和 link:overview_of_the_arl_upgrade.html["ARL 升級序列"]。
...
* 如果您要從現有系統升級，且升級過程中的停機時間包含在link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["支援的系統對照表"]中，則可以透過移動儲存設備或聯絡技術支援來升級控制器硬體。請參閱link:other_references.html["參考資料"]以連結至_透過移動磁碟區或儲存設備進行升級_。
