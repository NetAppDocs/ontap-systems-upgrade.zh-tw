...
[role="lead"]
有多種 Aggregate 重新配置（ARL）方法可用於升級控制器硬體。本程序說明如何透過將現有系統轉換為替換系統來升級 HA 配對中的儲存控制器，同時保留現有系統機箱、磁碟以及儲存在磁碟上的資料。
...
NOTE: 本文檔中，*node1* 和 *node2* 僅用作節點名稱的指涉。在執行操作步驟時，您必須使用節點的實際名稱。
...
* 此程序較為複雜，並假設您具備進階 ONTAP 管理技能。在開始升級之前，您應該閱讀並理解 link:guidelines-for-upgrading-controllers-with-arl.html["控制器升級指南"] 和 link:overview-of-the-arl-upgrade.html["ARL 升級序列"]。
...
* 如果您使用的交換器不受 ONTAP 版本和您要升級到的替代系統的支援，請參閱link:other-references.html["參考資料"]以連結至 _Hardware Universe_。
...
* 如果您要從現有系統升級，且升級過程中的停機時間包含在link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["支援的系統對照表"]中，則可以透過移動儲存設備或聯絡技術支援來升級控制器硬體。請參閱link:other-references.html["參考資料"]以連結至_透過移動磁碟區或儲存設備進行升級_。




[[automated-controller-upgrade-process]]
== 自動化控制器升級程序

此程序提供自動化程序的步驟、此程序使用自動磁碟指派和網路連接埠連線檢查來簡化控制器升級體驗。

.接下來呢？
在了解 ARL 升級程序之後，link:decide-to-use-the-aggregate-relocation-guide.html["決定是否使用此程序"]。
