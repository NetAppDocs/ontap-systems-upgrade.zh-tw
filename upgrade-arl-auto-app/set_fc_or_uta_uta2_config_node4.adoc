...
.關於這項工作
您可能需要完成<<configure-fc-ports-on-node4,在節點4上設定FC連接埠>>或<<check-config-uta-uta2-node4,檢查並設定節點4上的UTA/UTA2連接埠>>，或兩者皆需完成。

[NOTE]
====
如果節點 4 沒有板載 FC 連接埠、板載 UTA/UTA2 連接埠或 UTA/UTA2 卡（例如，ONTAP 9.15.1 或更新版本中引入的 AFF 和 FAS 系統），且您正在升級具有儲存磁碟的系統，則可以跳至link:reassign-node2-disks-to-node4.html["將 node2 磁碟重新指派給 node4"]。
...
.關於這項工作
如果您的系統沒有 FC 組態，則可以跳過此部分。如果您的系統有板載 UTA/UTA2 連接埠或 UTA/UTA2 轉接器，則可以在<<check-config-uta-uta2-node4,檢查並設定節點4上的UTA/UTA2連接埠>>中進行設定。
...
. [[step8]請採取下列其中一項行動：
+
** 如果 node4 有 UTA/UTA2 卡或 UTA/UTA2 內建連接埠，請前往 <<check-config-uta-uta2-node4,檢查並設定節點4上的UTA/UTA2連接埠>>。
...
[[step-2-check-and-configure-utauta2-ports-on-node4]]
[[check-config-uta-uta2-node4]]
== 步驟 2：檢查並設定 node4 上的 UTA/UTA2 連接埠
...
.接下來呢？
* 如果您要升級至 AFF A800 系統，請前往link:reassign-node2-disks-to-node4.html#reassign-node2-node4-app-step9["將節點 2 磁碟重新指派給節點 4 ，步驟 9"]。
