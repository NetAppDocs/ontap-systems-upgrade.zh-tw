...
. [[auto_install4_step22]]如果您安裝了NetApp儲存加密 (NSE) 驅動器，請執行下列步驟：
+
...
+
.接下來呢？
** 如果您有 FC 或 UTA/UTA2 配置，link:set_fc_or_uta_uta2_config_node4.html["設定並配置節點 4 上的 FC 或 UTA/UTA2 連接埠"] 。
** 如果您沒有 FC 或 UTA/UTA2 配置，link:reassign-node2-disks-to-node4.html#reassign-node2-node4-app-step1["將 node2 磁碟重新分配給 node4，步驟 1"]這樣node4就可以辨識node2的磁碟。
** 如果您有MetroCluster配置，link:set_fc_or_uta_uta2_config_node4.html["設定並配置節點 4 上的 FC 或 UTA/UTA2 連接埠"]檢測連接到節點的磁碟。
