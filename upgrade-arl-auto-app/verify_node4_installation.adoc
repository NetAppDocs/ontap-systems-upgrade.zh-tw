...
如果 node2 上的實體連接埠沒有直接對應到 node4 上的實體連接埠，則必須使用後續章節<<restore_network_configuration_on_node4,還原節點4上的網路組態>>來修復網路連線。
...
確認 node4 已達法定人數。然後，繼續執行控制器更換作業。
...
| 9.12.1 或更新版本 | 跳過此步驟、前往 <<verify_node4_step5,步驟5.>>。 
|===
+
...
[[restore_network_configuration_on_node4]]
