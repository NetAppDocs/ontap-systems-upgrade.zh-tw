...
.關於這項工作
您可能需要完成 <<configure-fc-ports-on-node3,在節點3上設定FC連接埠>>、<<check-config-uta-uta2-node3,檢查並設定節點3上的UTA/UTA2連接埠>> 或兩者。


NOTE: NetApp行銷資料可能會使用「UTA2」一詞來指代整合式網路卡（CNA）介面卡和連接埠。不過、CLI使用「CNA」一詞。

如果節點 3 沒有板載 FC 連接埠、板載 UTA/UTA2 連接埠或 UTA/UTA2 卡（例如 ONTAP 9.15.1 或更新版本中引入的 AFF 和 FAS 系統），且您正在升級具有儲存磁碟的系統，則可以跳至link:reassign-node1-disks-to-node3.html["將 node1 磁碟重新指派給 node3"]。



[[step-1-configure-fc-ports-on-node3]]
[[configure-fc-ports-on-node3]]
== 步驟 1：在 node3 上設定 FC 連接埠
...
.關於這項工作
如果您的系統沒有 FC 組態，則可以跳過此部分。如果您的系統有板載 UTA/UTA2 連接埠或 UTA/UTA2 卡，則可以在<<check-config-uta-uta2-node3,檢查並設定節點3上的UTA/UTA2連接埠>>中進行設定。
...
| 具有 UTA/UTA2 卡或 UTA/UTA2 板載端口 | 前往 <<check-config-uta-uta2-node3,檢查並設定節點3上的UTA/UTA2連接埠>> 
...
[[step-2-check-and-configure-utauta2-ports-on-node3]]
[[check-config-uta-uta2-node3]]
== 步驟 2：檢查並設定 node3 上的 UTA/UTA2 連接埠
