...
* 不過，如果您的 V 系列系統或系統配備 FlexArray 虛擬化軟體與儲存陣列，而 node3 則沒有內建 FC 連接埠，內建 UTA/UTA 連接埠或 UTA/UTA2 卡，請返回 _ 安裝與開機 node3_ ，然後繼續執行中的程序link:install_boot_node3.html#auto_install3_step23["步驟23"]。
...
** 如果 node3 沒有 UTA/UTA2 卡或 UTA/UTA2 板載端口，請跳過 _Check 並在 node3_ 上配置 UTA/UTA2 端口，然後轉至link:reassign-node1-disks-to-node3.html["將 node1 磁碟重新指派給 node3"]。
...
| 是V系列系統、或具有FlexArray 「支援虛擬化的軟體」、並連接至儲存陣列  a| 
** 如果node3具有UTA/UTA2卡或UTA/UTA2內建連接埠、請前往 <<檢查並設定節點3上的UTA/UTA2連接埠>>。
** 如果 node3 沒有 UTA/UTA2 卡或 UTA/UTA2 板載端口，請跳過 _Check 並在 node3_ 上配置 UTA/UTA2 端口，然後返回至 _Install and boot node3_ 並繼續執行中的步驟link:install_boot_node3.html#auto_install3_step23["步驟23"]。
...
| 是V系列系統、或具有FlexArray 「支援虛擬化的軟體」、並連接至儲存陣列 | 返回到 _ 安裝並引導 node3_ ，然後繼續執行過程link:install_boot_node3.html#auto_install3_step23["步驟23"]。 
|===
. [[auto_checks3_step13]結束維護模式：
+
《停止》

. [[step14]] 執行將節點開機至開機功能表 `boot_ontap menu`。如果您要升級至 AFF A800 系統，請前往link:reassign-node1-disks-to-node3.html#reassign-node1-node3-app-step9["將 node1 磁碟重新指派給 node3 ，步驟 9"]。
