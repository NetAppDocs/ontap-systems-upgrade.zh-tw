...
您必須確保原節點的連接埠和 LIF 組態與您計劃在新節點上的組態相容。這是因為新節點在開機時會重播相同的組態，這表示當您開機 node3 時，ONTAP 會嘗試在 node1 上使用的相同連接埠上託管 LIF。

因此，如果節點 1 上的實體連接埠與節點 3 上的實體連接埠不直接對應，則需要進行軟體組態變更才能在開機後還原叢集、管理和網路連線。此外，如果節點 1 上的叢集連接埠與節點 3 上的叢集連接埠不直接對應，則在進行軟體組態變更以將叢集 LIF 託管在正確的實體連接埠上之前，節點 3 在重新開機後可能無法自動重新加入仲裁。
...
|===
+
有關取得此資訊的步驟，請參閱 link:record_node1_information.html["記錄node1資訊"]。
...
----
+
前往 link:other_references.html["參考資料"] 以連結至 _Network Management_ 或 _ONTAP 9 Command reference_ 以取得更多資訊。
