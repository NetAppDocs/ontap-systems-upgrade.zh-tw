...
. [[unset_mastwait_manipal]]如果在組態中使用自我加密磁碟機、且您已設定 `kmip.init.maxwait` 變數至 `off` （例如、in link:install_boot_node3.html#step16["步驟16"] 在_Install and boot node3_）中、您必須取消設定變數：
+
