...
.. 確認新的叢集連接埠位於叢集廣播網域中：
+
「網路連接埠show -node<node>-port <port>-Fields broadcast網域'下列範例顯示連接埠「e0a」位於節點4的叢集網域中：
...
+
「MetroCluster 功能伺服器重新同步-vserver <vserver_name>」

.. 針對每個LIF、將叢集生命區移轉至新的連接埠一次：
+
「網路介面移轉-vserver叢集-lif<lif_name>-source-Node node4 - dest-node4 -destite-port <port_name>」
...
. 調整所有LIF容錯移轉群組：「網路介面修改-容錯移轉群組<容錯移轉群組>-容錯移轉原則<容錯移轉原則>」
+
