...
. 如果在組態中使用NetApp儲存加密（NSE）、而您使用「kmip.init.maxwait>`」變數「Off」（在_Boot node2中、搭配AFF 使用VA900控制器模組和NVRAM模組）、將「setenv bootarg.storageEncryption.supports」命令設為「true」、 link:boot_node2_with_a900_controller_and_nvs.html#boot_node2_step1["步驟1."]）、您必須重設變數：
+
