...
[role="lead"]
在交換相應AFF 的E4A900 node2控制器模組和NVRAM之後、您可能需要將模組以netboot方式開機。「netboot」一詞表示您是從ONTAP 儲存在遠端伺服器上的鏡像進行開機。準備網路開機時、您必須將ONTAP 一份《支援》9開機映像複本放到系統可以存取的網路伺服器上。
...
. 等待AFF 節點2現在在Escea900控制器模組上執行、以開機並顯示開機功能表選項、如下列輸出所示：
+
[listing]
----
...
+
「etenv PARTNER-sysid <node1_sysid>'」
