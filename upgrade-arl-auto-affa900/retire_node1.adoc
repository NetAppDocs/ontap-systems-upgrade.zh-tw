...
[role="lead"]
若要淘汰節點1、請繼續執行自動化作業、以停用與節點2的HA配對、並正確關閉節點1。您稍後必須從AFF 節點1機箱中移除EsceeERA700控制器模組和NVRAM模組、然後在AFF 節點1上安裝EsceA900 NVRAM模組和控制器模組。
...
=== 移除AFF 功能不再需要的NVRAM模組

請依照下列程序移除AFF 「VA700 NVRAM模組」。


NOTE: 此功能可在插槽6中使用AFF 、高度是系統中其他模組的兩倍。
...
. 如果AFF 您使用任何附加模組做為ESIEA700 NVRAM模組上的coredump裝置、請勿將其傳輸至AFF ESIA900 NVRAM模組。
...
您必須在AFF 節點1上安裝收到的用於升級的EsceA900 NVRAM模組和控制器模組。請勿將coredump裝置從AFF ESIEA700 NVRAM模組移至AFF ESIEA900 NVRAM模組。
...
請使用下列程序、將AFF E4A900 NVRAM模組安裝在節點1的插槽6中。
...
=== 在AFF 節點1上安裝E4A900控制器模組。

請使用下列程序、在AFF 節點1中安裝EsceA900控制器模組。
