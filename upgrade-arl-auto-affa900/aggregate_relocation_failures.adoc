...
+
如需有關「強制集合重新定位開始」、「置換-否決」和「置換-目的地檢查」選項的詳細資訊、請參閱 link:other_references.html["參考資料"] 若要連結至「_SURF9 ONTAP 命令：手動頁面參考_」。
...
* 在第4階段之後、當節點2被AFF 取代為EsceA900控制器模組和NVRAM模組時。
+
...
. 取得Aggregate清單：
+
「torage Aggregate show -nodes <node2>-is主目錄true」
...
. 重新定位節點2上留下的集合體：
+
「torage Aggregate regate regulate regulationstart -node2>-aggr <aggr_node_1>-destination <node1>'」
