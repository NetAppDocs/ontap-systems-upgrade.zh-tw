...
[cols="35,65"]
|===
| 現有 node1 控制器 | 更換 node1 系統模組 


| 解答800 AFF | AFF A90 或 AFF A70 ^1^ 


| AFF A220 已設定為 ASA | AFF A150 控制器模組 ^1^ 
...
| AFF A700 設定為 ASA | ASA A900 控制器與 NVRAM 模組 ^2^ 
...
.步驟
. （僅限 AFF A800 升級）在載入程式提示下、進入維護模式：
+
Boot_ONTAP maint
...
. [[A900_boot_node1]] 如果您安裝了 NetApp 儲存加密（ NSE ）磁碟機、請執行下列步驟。
+
