...
[role="lead"]
節點1搭配AFF EsceA900控制器模組和NVRAM模組、現在已準備好進行開機。透過交換控制器模組和NVRAM模組、將AFF 某個處理器從某個部分升級至AFF 某個部分、只需移動主控台和管理連線即可。本節提供使用AFF E4A900控制器模組和NVRAM模組來開機節點1所需的步驟。
...
. 節點會在開機功能表停止。輸入「22/7」、然後選取隱藏選項「boot_after控制器置換」。若要將AFF EsceA700 node1磁碟重新指派至AFF EsceA900 node1、請在提示字元下輸入節點1的實際節點名稱。請參考下列範例：
+
[listing]
----
