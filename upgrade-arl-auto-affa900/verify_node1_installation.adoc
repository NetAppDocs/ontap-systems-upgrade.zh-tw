...
[role="lead"]
您必須使用AFF EsceA900控制器模組和NVRAM模組來驗證node1安裝。由於實體連接埠沒有變更、因此您不需要將實體連接埠從AFF EsceA700 node1對應到AFF EsceA900 node1。

使用AFF E4A900控制器模組開機節點1之後、您必須確認其安裝正確。您必須等待節點1加入仲裁、然後繼續執行控制器更換作業。
...
NOTE: 在將介面升級至Ard21控制器期間AFF 、網路連接埠及其連線功能不應改變。AFF所有連接埠都應位於正確的廣播網域中、而且網路連接埠的連線能力不應改變。不過、在將LIF從節點2移回節點1之前、您必須先確認網路連接埠的連線能力和健全狀況狀態。
