...
.步驟
. [[A900_boot_node2].如果此組態使用NetApp儲存加密（NSE）、則必須將「show bootenv bootarg.storageEncryption.support'命令設定為「true」、並將「kmip.init.maxwait` variable」設定為「Off」、以避免在節點2組態載入後發生開機迴圈：
+
