...
.步驟
. [[boot_node2_step1]]設定 `bootarg.storageencryption.support` 和 `kmip.init.maxwait` 在節點2組態載入後、避免開機迴圈的變數。
+
...
WARNING: *只要在HA配對上完成控制器升級、您就必須取消設定 `kmip.init.maxwait` 變動。請參閱 link:ensure_new_controllers_are_set_up_correctly.html#unset_maxwait["請確認新控制器的設定正確、步驟12"]。如果發生停電、否則可能導致資料遺失。*
