...
.. 在執行可達性修復時， ONTAP 會嘗試將連接埠放置在正確的廣播網域中。但是，如果 ONTAP 無法確定連接埠的可達性，且該連接埠不屬於任何現有廣播網域， ONTAP 會為這些連接埠建立新的廣播網域。如有需要，您可以刪除新建立的廣播網域，前提是其所有成員連接埠都將成為介面群組的成員連接埠。刪除廣播網域：
+
...
. 從 ONTAP 9.8 開始，如果連接埠在網路連接埠可連線性修復程序期間於廣播網域之間移動，ONTAP 會自動修改 LIF 的主連接埠。如果 LIF 的主連接埠已移至另一個節點或未指派，該 LIF 會顯示為已移出的 LIF。還原主連接埠已不存在或已重新配置至另一個節點之已移出 LIF 的主連接埠。
+
