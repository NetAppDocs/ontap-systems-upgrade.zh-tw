...
summary: 在將 node2 轉換為磁碟機櫃之前，請將 node2 上的邏輯介面（LIF）和資料集合體移轉至 node1。 
---
...
[role="lead"]
在將節點 2 轉換為磁碟機櫃之前，將 AFF A150、AFF A220 或 FAS2820 節點 2 上的邏輯介面（LIF）和資料集合體遷移到節點 1。

.開始之前
確認您符合下列要求：

* 原控制器和替換控制器應盡可能使用相同的 ONTAP 版本和修補程式版本。請參閱 https://hwu.netapp.com["NetApp Hardware Universe"^]以了解支援的 ONTAP 版本。
...
.接下來呢？
link:convert-node2-to-drive-shelf.html["將 node2 轉換為磁碟機櫃、並連線至 node4"]
