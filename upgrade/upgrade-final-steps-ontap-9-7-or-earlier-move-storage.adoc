...
.開始之前
這些步驟適用於執行ONTAP 下列版本的系統：如果您執行ONTAP 的是更新版本的版本、則必須使用中的程序 link:upgrade-final-upgrade-steps-in-ontap-9-8.html["執行ONTAP 更新版本的最終升級步驟"]。

.步驟
. 如果您的系統執行ONTAP 的是*停止*版本9.8或更新版本。您必須使用中的程序 link:upgrade-final-upgrade-steps-in-ontap-9-8.html["請執行ONTAP 更新版本的最終升級步驟"]。
