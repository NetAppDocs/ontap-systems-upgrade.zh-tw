...
summary: '如果需要，在升級至 ONTAP 9.15.1 或更新版本中引入的控制器時，可使用 `system controller replace`命令在 node3 上還原金鑰管理程式組態。' 
---
= 還原節點3上的金鑰管理程式組態
:hardbreaks:
:allow-uri-read: 
...
.接下來呢？
在節點 3 上還原金鑰管理程式組態之後link:move_non-root_aggr_and_nas_data_lifs_node1_from_node2_to_node3.html["將節點 1 擁有的非根 Aggregate 和 NAS 資料 LIF 從節點 2 移至節點 3"]。
