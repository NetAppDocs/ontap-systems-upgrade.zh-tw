...
summary: '升級至 ONTAP 9.15.1 或更新版本中引入的控制器時，使用 `system controller replace`命令手動將失敗或被否決的集合體重新定位至 node2。' 
---
= 將失敗或被否決的聚合重新定位到節點 2
:hardbreaks:
:allow-uri-read: 
...
.接下來呢？
將失敗或被否決的 Aggregate 重新配置到 node2 後，link:retire_node1.html["退役 node1"]。
