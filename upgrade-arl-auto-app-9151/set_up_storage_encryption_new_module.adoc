...
summary: '如果需要，可在升級至 ONTAP 9.15.1 或更新版本所引入的控制器時，使用 `system controller replace`命令來設定儲存加密、安裝 SSL 憑證及設定金鑰管理伺服器。' 
---
= 在新的控制器模組上設定儲存加密
:hardbreaks:
:allow-uri-read: 
...
.接下來呢？
在新控制器模組上設定儲存加密後，link:set_up_netapp_volume_encryption_new_module.html["在新控制器模組上設定 NetApp Volume 或 Aggregate Encryption"]。
