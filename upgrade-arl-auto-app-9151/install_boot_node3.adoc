...
summary: '使用 `system controller replace`命令安裝並啟動 node3，並在升級到 ONTAP 9.15.1 或更新版本中引入的控制器時安裝 ONTAP 軟體。' 
---
= 安裝及開機節點3
:hardbreaks:
:allow-uri-read: 
...
. 當 node3 停止在啟動選單時，透過在 node3 上執行以下命令將 node1 磁碟重新指派給 node3：
+
...
. 在自動開機期間輸入  `Ctrl-C` 以在  `LOADER>` 提示字元處停止節點。
. 在載入程式提示下、進入維護模式：
+
Boot_ONTAP maint
...
.接下來呢？
安裝並啟動 node3 後，link:verify_node3_installation.html["驗證 node3 安裝"]。
