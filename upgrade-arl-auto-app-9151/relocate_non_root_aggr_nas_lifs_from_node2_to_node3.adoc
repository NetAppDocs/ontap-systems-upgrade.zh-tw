...
summary: '使用  `system controller replace` 命令，將 node2 的非根 Aggregate 重新定位至 node3，以升級至 ONTAP 9.15.1 或更新版本中引入的控制器。' 
---
...
.接下來呢？
將非根 Aggregate 和 NAS 資料 LIF 從節點 2 重新配置到節點 3 後，link:relocate-failed-or-vetoed-aggr-node3.html["將失敗或被否決的 Aggregate 重新配置到 node3"]
