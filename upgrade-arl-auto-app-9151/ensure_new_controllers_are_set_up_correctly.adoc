...
summary: '啟用 HA 配對、驗證 node3 和 node4 儲存設備存取、並在使用命令升級執行 ONTAP 9.15.1 或更新版本的控制器之後、確認 Aggregate 擁有權 `system controller replace` 。' 
---
= 確認新的控制器已正確設定
:hardbreaks:
:allow-uri-read: 
...
. [unset_maxwait_system_commands] 如果在組態中使用自我加密磁碟機、而且您已將變數設 `kmip.init.maxwait` 為 `off` （例如、在中 link:install_boot_node4.html#auto_install4_step24["安裝並開機節點4、步驟24"]）、則必須取消設定變數：
+
...
+
請參閱 link:other_references.html["參考資料"] 以連結至《系統管理參考》，以取得有關 SP 的資訊，以及《ONTAP 9 命令參考》，以取得有關系統  `service-processor network modify` 命令的詳細資訊。
...
.接下來呢？
確認新控制器設定正確後，link:set_up_storage_encryption_new_module.html["在新的控制器模組上設定 Storage Encryption"]。
