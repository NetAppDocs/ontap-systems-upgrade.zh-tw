...
summary: '使用 `system controller replace`命令準備節點，並在升級至 ONTAP 9.15.1 或更新版本中引入的控制器時執行一系列預先檢查。' 
---
= 準備節點以進行升級
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
控制器更換程序首先會進行一系列預先檢查。您也會收集原始節點的相關資訊、以便稍後在程序中使用。如果有任何預先檢查失敗、您必須先修正 Aggregate 擁有權、才能繼續進行升級。
...
啟動控制器更換流程，並對節點 1 和節點 2 執行預檢查。如有必要，請確定正在使用的自加密磁碟機的類型。
...
[NOTE]
====
您只能在進階權限層級執行 `system controller replace start`命令：
...
====
+
您會看到類似以下範例的輸出。此輸出顯示在叢集上執行的 ONTAP 版本：
...
| AutoSupport 狀態檢查 | 如果AutoSupport 未設定顯示功能通知、工作就會失敗。在AutoSupport 開始更換控制器之前、您必須先啟用支援功能。 
...
檢查 node3 和 node4 上的建置是否為 NODAR （無靜態資料加密）映像。如果建置為 NODAR 映像，則需要安裝 DARE （靜態資料加密）映像。
...
----
+
在命令輸出中，檢查 `build-type`是否為 NODAR 影像，例如 `DEBUG SIM NODAR`。

+

NOTE: 如果 `build-type`不是 NODAR 鏡像，則建置版本為 DARE 鏡像。

. 如果建置的是 NODAR 映像，則安裝 DARE 映像。
+
.. 退出系統 shell 和啟動選單：
+
[source, cli]
...
halt
----
.. 安裝 DARE 映像。如需更多資訊，請參閱知識庫文章 link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/Netbooting_a_FAS_and_AFF_Controller_in_ONTAP_9["在 ONTAP 9 中透過網路啟動 FAS 和 AFF 控制器"^]。




.接下來呢？
準備好節點後，link:complete-preparation-for-upgrade.html["完成升級準備工作"]。
