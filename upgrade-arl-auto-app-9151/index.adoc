...
[role="lead"]
有多種 Aggregate 重新配置（ARL）方法可用於升級控制器硬體。本 ARL 程序說明如何在 ONTAP 9.15.1 或更新版本中推出的 AFF 和 FAS 儲存系統中，使用新控制器升級 HA 配對控制器，同時保留現有磁碟及磁碟上儲存的資料。
...
.自動化控制器升級程序
在控制器升級期間、現有控制器會由執行較新或更強大平台的另一個控制器取代。本內容提供部分自動化程序的步驟、該程序利用自動網路連接埠連線能力檢查、進一步簡化控制器升級體驗。

.接下來呢？
在了解 ARL 升級程序之後，link:decide_to_use_the_aggregate_relocation_guide.html["決定是否使用此程序"]。
