...
summary: '了解是否可以使用 `system controller replace`命令升級至 ONTAP 9.15.1 或更新版本中引入的控制器。' 
---
= 使用ARL升級控制器的準則
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
若要了解是否可以使用 ARL 升級至 ONTAP 9.15.1 或更新版本中引入的一對控制器，取決於平台以及原始控制器和替換控制器的組態。
...
如果在升級控制器過程中出現任何問題，請參閱link:aggregate_relocation_failures.html["疑難排解"]了解更多資訊和可能的解決方案。

如果您找不到解決所遇到問題的解決方案、請聯絡技術支援部門。

.接下來呢？
在查閱使用 ARL 升級控制器的準則後link:overview_of_the_arl_upgrade.html["瞭解 ARL 升級順序"]。
