...
summary: '決定是否使用 ARL 搭配 `system controller replace`指令升級至 ONTAP 9.15.1 或更新版本中引入的控制器。' 
---
= 決定是否使用此聚合遷移程序
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
有多種聚合重新定位 (ARL) 方法可用於升級控制器硬體。本 ARL 程序描述如何在 AFF 和 FAS 儲存系統中將一對高可用性 (HA) 控制器升級至 ONTAP 9.15.1 或更新版本中引入的控制器，同時保留現有磁碟及其上儲存的資料。這是一個複雜的程序，僅應由經驗豐富的 ONTAP 管理員執行。
...
|===
+
若要執行上表中所列的升級，請參閱 link:other_references.html["參考資料"] 以連結至「使用 "system controller replace" 命令升級同一機箱中的控制器型號」。
...
.接下來呢？
在決定使用此 ARL 程序之後，link:required_tools_and_documentation.html["瞭解所需的工具和文件"]。
