...
summary: '透過使用 `system controller replace`命令，驗證 node1 實體連接埠是否正確對應到 node3，並在升級到 ONTAP 9.15.1 或更新版本中引入的控制器時，還原 node3 上的網路組態。' 
---
= 驗證node3安裝
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
您必須驗證 node1 的實體連接埠是否正確對應到 node3 的實體連接埠。這樣才能確保升級後 node3 能夠與叢集中的其他節點以及網路進行通訊。

.關於這項工作
請參閱 link:other_references.html["參考資料"] 可鏈接到_SURE_ Hardware Universe 以獲取有關新節點上端口的信息。您將會使用本節稍後的資訊。

實體連接埠配置可能因節點機型而異。新節點開機時，ONTAP 會嘗試判斷哪些連接埠應裝載叢集 LIF，以便自動進入仲裁。

如果 node1 上的實體連接埠無法直接對應到 node3 上的實體連接埠，則必須使用後續章節<<restore_network_configuration_on_node3_9151,還原節點3上的網路組態>>來修復網路連線。

安裝並開機節點3之後、您必須驗證節點是否已正確安裝。您必須等待節點3加入仲裁、然後繼續重新配置作業。

在此程序中，由於 node3 加入仲裁，作業會暫停。



[[step-1-verify-that-node3-is-in-quorum]]
== 步驟 1：驗證 node3 是否在仲裁中

確認 node3 已達法定人數。然後，繼續執行控制器更換作業。
...
[[step-2-restore-network-configuration-on-node3]]
[[restore_network_configuration_on_node3_9151]]
== 步驟 2：還原 node3 上的網路組態
...
+
在執行可達性修復時，ONTAP 會嘗試將連接埠放置在正確的廣播網域中。但是，如果 ONTAP 無法確定連接埠的可達性，且該連接埠不屬於任何現有廣播網域，ONTAP 會為這些連接埠建立新的廣播網域。
...
.接下來呢？
驗證 node3 安裝後，link:restore_key-manager_configuration_node3.html["在節點 3 上還原金鑰管理程式組態"]。
