...
summary: '如果需要，升級至 ONTAP 9.15.1 或更新版本中引入的控制器時，可以使用 `system controller replace`命令在 node4 上還原金鑰管理器組態。' 
---
= 還原節點4上的金鑰管理程式組態
:hardbreaks:
:allow-uri-read: 
...
.接下來呢？
在 node4 上還原金鑰管理程式組態之後link:move_non_root_aggr_and_nas_data_lifs_node2_from_node3_to_node4.html["將節點 2 擁有的非根 Aggregate 和 NAS 資料 LIF 從節點 3 移至節點 4"]。
