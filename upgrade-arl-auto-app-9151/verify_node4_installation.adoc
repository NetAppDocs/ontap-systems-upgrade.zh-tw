...
summary: '驗證 node2 的實體連接埠是否正確對應到 node4，並在升級至 ONTAP 9.15.1 或更新版本所引入的控制器時，使用 `system controller replace`命令還原 node4 上的網路組態。' 
---
= 驗證node4安裝
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
您必須驗證 node2 的實體連接埠是否正確對應到 node4 的實體連接埠。這樣才能確保升級後 node4 能夠與叢集中的其他節點以及網路進行通訊。
...
如果 node2 上的實體連接埠沒有直接對應到 node4 上的實體連接埠，則必須使用後續章節<<restore_network_configuration_on_node4_9151,還原節點4上的網路組態>>來修復網路連線。
...
[[step-1-verify-that-node4-is-in-quorum]]
== 步驟 1：驗證 node4 是否在仲裁中

確認 node4 處於仲裁狀態。然後，繼續執行控制器更換作業。
...
[[step-2-restore-network-configuration-on-node4]]
[[restore_network_configuration_on_node4_9151]]
== 步驟 2：還原 node4 上的網路組態
...
.接下來呢？
驗證 node4 安裝後，link:restore_key-manager_config_node4.html["在節點 4 上還原金鑰管理程式組態"]。
