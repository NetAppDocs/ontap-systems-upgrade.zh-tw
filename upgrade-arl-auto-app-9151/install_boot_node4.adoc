...
summary: '使用 `system controller replace`命令安裝並啟動 node4，並在升級到 ONTAP 9.15.1 或更新版本中引入的控制器時安裝 ONTAP 軟體。' 
---
= 安裝及開機節點4
:hardbreaks:
:allow-uri-read: 
...
. 纜線節點4、將連線從節點2移至節點4。
+
依照 node4 平台的《安裝與設定說明》、對應的磁碟架文件和《HA 對管理》中的說明，連接以下連接線。
...
. [[auto_check_4_step11]]當 node4 停止在啟動選單時，透過在 node4 上執行下列命令將 node2 磁碟重新指派給 node4：
+
...
. [[auto_check_4_step27]]在 LOADER 提示字元下，啟動節點：
+
Boot_ONTAP功能表
...
.接下來呢？
安裝並啟動 node4 後，link:verify_node4_installation.html["驗證 node4 安裝"]。
