...
summary: '使用 `system controller replace`命令，在升級至 ONTAP 9.15.1 或更新版本中引入的控制器時，手動將失敗或被否決的 Aggregate 重新定位至 node3。' 
---
= 將失敗或被否決的聚合重新定位到節點 3
:hardbreaks:
:allow-uri-read: 
...
.接下來呢？
將失敗或被否決的 Aggregate 重新配置到 node3 後link:retire_node2.html["退役 node2"]。
