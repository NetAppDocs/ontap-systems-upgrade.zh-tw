...
summary: '使用 `system controller replace`指令將非根聚合和 NAS 資料 LIF 從節點 1 移至節點 2，並在升級至 ONTAP 9.15.1 或更新版本所引入的控制器時，將節點 1 的資源移至節點 3。' 
---
...
.接下來呢？
將節點 1 擁有的非根 Aggregate 和 NAS 資料 LIF 重新配置到節點 2 後link:relocate-failed-or-vetoed-aggr-node2.html["將失敗或被否決的 Aggregate 重新配置到 node2"]。
